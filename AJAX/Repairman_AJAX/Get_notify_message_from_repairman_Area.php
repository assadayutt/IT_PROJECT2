...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['area_id'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT Area_Assign_work.message_work, Area_repair.area_detail, Area_repair.area_problem, User.user_linetoken, Repairman.repairman_name, Area_Assign_work.assign_datecomp
        FROM Area_repair 
