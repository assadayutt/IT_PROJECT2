...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt = $conn->prepare("SELECT Officer.Line_Token, Repairman.repairman_name, Approve_Request_Tools.date, Approve_Request_Tools.approve_id
    FROM Approve_Request_Tools
    JOIN Officer ON Approve_Request_Tools.officer_id = Officer.officer_id
    JOIN Repairman ON Approve_Request_Tools.repairman_id = Repairman.repairman_id
    WHERE Approve_Request_Tools.repairman_id = :repairman_id
    ORDER BY Approve_Request_Tools.approve_id DESC
    LIMIT 1;");
